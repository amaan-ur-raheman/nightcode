...
describe("generateSessionTitle", () => {
    ...
    it("falls back to truncated message when GROQ_API_KEY is missing", async () => {
        ...
        const { generateSessionTitle } = await import("../generate-session-title");
        const title = await generateSessionTitle("Hello, this is a test message for my coding session");
        expect(title).toBe("Hello, this is a test message for my coding session");
    });
    ...
    it("uses Groq API when key is available", async () => {
        ...
        const { generateSessionTitle } = await import("../generate-session-title");
        const title = await generateSessionTitle("Fix authentication bug in login flow");

...
import { buildSystemPrompt, buildSubagentSystemPrompt } from "../../system-prompt";
...
describe("buildSystemPrompt", () => {
    ...
    it("caches results for same parameters", () => {
        const first = buildSystemPrompt({ mode: "BUILD", currentModel: "gpt-4o" });
        const second = buildSystemPrompt({ mode: "BUILD", currentModel: "gpt-4o" });
    ...
    it("includes current model name in spawnAgent description", () => {
        const prompt = buildSystemPrompt({ mode: "BUILD", currentModel: "claude-sonnet-4-20250514" });
        expect(prompt).toContain("claude-sonnet-4-20250514");
    });

    it("includes parallel execution guidance in PLAN mode spawning section", () => {
        const prompt = buildSystemPrompt({ mode: "PLAN" });
    ...
    it("includes parallel execution guidance in BUILD mode spawning section", () => {
        const prompt = buildSystemPrompt({ mode: "BUILD" });
        expect(prompt).toContain("Parallel Execution");

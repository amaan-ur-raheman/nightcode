...
describe('getFallbackChain', () => {
    it('returns NIM fallbacks for NIM models', () => {
        const chain = getFallbackChain('nvidia/deepseek-ai/deepseek-v4-pro');
    ...
    it('returns Anthropic fallbacks for Anthropic models', () => {
        ...
        expect(chain).toEqual(['gpt-4o', 'nvidia/nemotron-3-ultra-550b-a55b']);
    });
...
describe('withFallback', () => {
    ...
    it('stops on auth errors (401/403)', async () => {
        ...
        await expect(withFallback(fn, 'primary-model')).rejects.toThrow(
            '401 unauthorized',
        );
        expect(fn).toHaveBeenCalledTimes(1);
    });
    ...
    it('throws last error after exhausting retries', async () => {
        ...
        await expect(withFallback(fn, 'primary-model', 1)).rejects.toThrow(
            'server error',
        );

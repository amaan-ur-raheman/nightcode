...
describe("NIM Provider", () => {
    ...
    it("throws when NIM_API_KEY is not set", async () => {
        ...
        await expect(nim("deepseek-ai/deepseek-v4-flash")).rejects.toThrow("NIM_API_KEY");
    });
    ...
    it("uses NIM_API_KEY when set", async () => {
        ...
        await expect(nim("deepseek-ai/deepseek-v4-flash")).resolves.toBeDefined();
    });
    ...
    it("nimSubagent uses subagent key when available", async () => {
        ...
        const result = await nimSubagent("deepseek-ai/deepseek-v4-flash");
    ...
    it("nimSubagent falls back to NIM_API_KEY when subagent key not set", async () => {
        ...
        const { nimSubagent } = await import("../nim");
        await expect(nimSubagent("deepseek-ai/deepseek-v4-flash")).resolves.toBeDefined();
    });
});

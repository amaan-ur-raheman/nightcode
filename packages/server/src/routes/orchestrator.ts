...
const app = new Hono<AuthenticatedEnv>()
    .post(
        ...
        async (c) => {
            ...
            console.log(`[orchestrator:${reqId}] → Decompose: model=${model} mode=${mode} strategy=${strategy} messages=${messages.length}`);

            const creditsBalance = await getAvailableCreditsBalance(userId);
            if (creditsBalance <= 0) {
                console.log(`[orchestrator:${reqId}] ✗ No credits remaining for user ${userId.slice(0, 8)}`);
            ...
            const strategyHints: Record<string, string> = {
                balanced: "Create 3-5 tasks with reasonable parallelism. Favor clean dependency chains.",
                speed: "Maximize parallelism — run independent tasks simultaneously. Use 3-4 tasks.",
                quality: "Create 5-7 tasks with thorough review steps. Include dedicated reviewer and tester roles.",
            };
            ...
            console.log(`[orchestrator:${reqId}] ✓ Decomposed: ${text.length} chars provider=${usedProvider} model=${usedModelId}`);
            if (usage) {
                const tokens = (usage.inputTokens ?? 0) + (usage.outputTokens ?? 0);
                console.log(`[orchestrator:${reqId}]   Tokens: ${tokens} (${usage.inputTokens ?? 0} in / ${usage.outputTokens ?? 0} out)`);
                try {

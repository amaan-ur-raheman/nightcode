...
type IngestUsageForMeterParams = {
    messageId: string;
...
async function streamAIResponse(
    ...
    const persistInterruptedMessage = async () => {
        ...
        const validatedParts: Prisma.InputJsonValue | undefined = parts.length > 0
    ...
    const ingestUsageForMessage = async ({ messageId, status }: IngestUsageForMeterParams) => {

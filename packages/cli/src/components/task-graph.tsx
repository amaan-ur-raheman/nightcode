...
const STATUS_COLORS: Record<string, keyof ReturnType<typeof useTheme>["colors"]> = {
...
function TaskNodeRow({ node, compact }: { node: TaskNode; compact?: boolean }) {
    ...
    const topTools = node.toolsUsed
        ? Object.entries(node.toolsUsed).sort(([, a], [, b]) => b - a).slice(0, 3)
        : [];
            ...
            {node.status === "running" && node.currentTool && (
                <box flexDirection="row" gap={1} paddingLeft={2} width="100%">
                    <text attributes={TextAttributes.DIM} fg={colors.info}>
                        → {formatToolName(node.currentTool)}{node.currentToolInput ? ` ${node.currentToolInput}` : ""}
                    </text>
                </box>
            )}
...
export const TaskGraphView = React.memo(function TaskGraphView({
    graph,
    compact = false,
}: TaskGraphViewProps) {
    const { colors } = useTheme();
    ...
    return (
        <box flexDirection="column" gap={0} width="100%">
            ...
            <box flexDirection="column" gap={0}>
                {sorted.map((node) => (
                    <TaskNodeRow key={node.id} node={node} compact={compact} />
                ))}
            </box>
            {graph.completedAt && (
                <text attributes={TextAttributes.DIM} fg={colors.dimSeparator}>
                    Completed in {((graph.completedAt - graph.createdAt) / 1000).toFixed(1)}s
                </text>
            )}
        </box>
    );
}, (prev, next) => prev.graph.version === next.graph.version && prev.compact === next.compact);

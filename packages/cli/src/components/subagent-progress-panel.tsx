...
function StepProgress({ step, maxSteps, colors }: { step: number; maxSteps: number; colors: any }) {
    const width = 12;
    ...
    return (
        <text attributes={TextAttributes.DIM}>
            ...
            <text fg={colors.dimSeparator}> {step}/{maxSteps}</text>
        </text>
    );
}
...
function SubagentRow({ info, index }: { info: SubagentInfo; index: number }) {
    ...
    const isDone = info.status === "completed" || info.status === "failed" || info.status === "cancelled";

    const statusSymbol = info.status === "completed"
        ? "✓"
        : info.status === "failed"
            ? "✗"
    ...
    return (
        <box flexDirection="column" gap={0} width="100%">
            ...
            <box flexDirection="row" gap={1} paddingLeft={2} width="100%">
                <StepProgress step={info.step} maxSteps={info.maxSteps} colors={colors} />
                <text attributes={TextAttributes.DIM} fg={colors.dimSeparator}>
                    · {prettyMs(elapsed)}
                </text>
                {info.toolCallCount > 0 && (
                    <text attributes={TextAttributes.DIM} fg={colors.dimSeparator}>
                        · {info.toolCallCount} tool{info.toolCallCount !== 1 ? "s" : ""}
                    </text>
                )}
            </box>
            {!isDone && info.currentTool && (
                <box flexDirection="row" gap={1} paddingLeft={2} width="100%">
                    <text attributes={TextAttributes.DIM} fg={colors.info}>
                        → {formatToolName(info.currentTool)}{info.currentToolInput ? ` ${info.currentToolInput}` : ""}
                    </text>
                </box>
            )}
            ...
            {topTools.length > 0 && (
                <box flexDirection="row" gap={1} paddingLeft={2} width="100%" flexWrap="wrap">
                    {topTools.map(([tool, count]) => (
                        <text key={tool} attributes={TextAttributes.DIM} fg={colors.dimSeparator}>
                            {tool}×{count}
                        </text>
                    ))}
...
export const SubagentProgressPanel = React.memo(function SubagentProgressPanel({
    ...
    const { colors } = useTheme();
    const [subagents, setSubagents] = useState<SubagentInfo[]>(getActiveSubagents());
    ...
    const failed = filtered.filter((s) => s.status === "failed" || s.status === "cancelled");
    const totalToolCalls = filtered.reduce((sum, s) => sum + s.toolCallCount, 0);
        ...
        return (
            <box flexDirection="column" gap={0} paddingLeft={3} width="100%">
                ...
                {[...running, ...completed, ...failed].map((info) => {
                    ...
                    return (
                        <box key={info.id} flexDirection="column" gap={0} width="100%">
                            <box flexDirection="row" gap={1} width="100%">
                                <text fg={statusColor}>{statusSymbol}</text>
                                {info.currentTool && info.status === "running" ? (
                                    ...
                                    <text attributes={TextAttributes.DIM} fg={colors.dimSeparator}>
                                        {topTools.map(([t, c]) => `${t}×${c}`).join(" · ")}{totalTools > 3 ? ` +${totalTools - 3}` : ""}
                                    </text>
                                ) : info.status === "completed" ? (
                                    <text attributes={TextAttributes.DIM} fg={colors.dimSeparator}>done</text>
                                ) : info.status === "failed" ? (
                                    <text attributes={TextAttributes.DIM} fg={colors.error}>failed</text>
                                ) : null}
                            </box>
                        </box>
                    );
    ...
    return (
        <box flexDirection="column" gap={1} paddingLeft={3} paddingY={1} width="100%">
            {/* Summary header */}
            <box flexDirection="row" gap={1} width="100%">

        ...
        >
            ...
            >
                {Object.entries(fileGroups).map(
                    ([file, fileMatches], fileIdx) => {
                        const visibleMatches = fileMatches.slice(
                            0,
                            ...
                            >
                                ...
                                {visibleMatches.map((m, mIdx) => {
                                    const displayContent =
                                        m.content.length > MAX_LINE_LENGTH
                                            ? m.content.slice(
                                                  0,
                                                  MAX_LINE_LENGTH - 3,
                                              ) + '...'
                                ...
                                {remaining > 0 && (
                                    <box paddingLeft={2}>
                                        <text fg={colors.dimSeparator}>
                                            ... and {remaining} more match
                                            {remaining > 1 ? 'es' : ''} in this
                                            file
                                        </text>
                                    </box>
                                )}
        ...
        >
            ...
            >
                {currentBranch && (
                    <text fg={colors.info}>
                        On branch:{' '}
                        <span fg={colors.primary}>{currentBranch}</span>
                    </text>
                )}
                {hasStaged && (
                    <box flexDirection="column" marginTop={1} width="100%">
                        ...
                        {staged.length > MAX_FILES_SHOWN && (
                            <text paddingLeft={2} fg={colors.dimSeparator}>
                                ... and {staged.length - MAX_FILES_SHOWN} more
                                staged file(s)
                            </text>
                        )}
                    </box>
                ...
                {hasUnstaged && (
                    <box flexDirection="column" marginTop={1} width="100%">
                        ...
                        {unstaged.length > MAX_FILES_SHOWN && (
                            <text paddingLeft={2} fg={colors.dimSeparator}>
                                ... and {unstaged.length - MAX_FILES_SHOWN} more
                                unstaged file(s)
                            </text>
                        )}
                    </box>
                ...
                {hasUntracked && (
                    <box flexDirection="column" marginTop={1} width="100%">
                        ...
                        {untracked.length > MAX_FILES_SHOWN && (
                            <text paddingLeft={2} fg={colors.dimSeparator}>
                                ... and {untracked.length - MAX_FILES_SHOWN}{' '}
...
export function ProfileCodeBlock({
    ...
    const allItems =
        topPerformers && topPerformers.length > 0 ? topPerformers : hotspots;

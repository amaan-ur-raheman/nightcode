...
function getThemeColors(): ThemeColors {
    try {
        const prefs = JSON.parse(
            readFileSync(
                join(homedir(), '.nightcode', 'preferences.json'),
                'utf8',
            ),
        );
...
try {
    const raw = readFileSync(
        join(homedir(), '.nightcode', 'last-session'),
        'utf8',
    );
    ...
    if (id && title) {
        ...
        for (let row = 0; row < 4; row++) {
            ...
            const right = ['C', 'O', 'D', 'E']
                ...
                .join(' ');
            process.stdout.write(
                draw(left, primary, dimFg, '\x1b[48;5;235m') +
                    '  ' +
                    draw(right, planMode, dimFg, '\x1b[48;5;238m') +
                    '\n',
            );
        }
        process.stdout.write('\n');
        process.stdout.write(
            ` ${dim}${pad('Session')}${R}${bold}${title}${R}\n`,
        );
        process.stdout.write(
            ` ${dim}${pad('Continue')}${R}${bold}nightcode -s ${id}${R}\n`,
        );
        process.stdout.write('\n');
    }
} catch {

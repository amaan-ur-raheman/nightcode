...
if (args[0] === "init") {
    ...
    if (templateIdx !== -1) {
        ...
        if (val === "basic" || val === "fullstack" || val === "api") {
            ...
            console.error(`Error: Invalid template "${val ?? ""}". Allowed templates: basic, fullstack, api`);
            process.exit(1);
        }
    }
    ...
    for (let i = 1; i < args.length; i++) {
        ...
        if (arg === "--template") { i++; continue; } // skip flag + value
...
if (args[0] === "mcp") {
    const { mcpAddCommand, mcpRemoveCommand, mcpListCommand } = await import("./commands/mcp");
    ...
    switch (subcommand) {
        case "add": {
            ...
            const command = commandIdx !== -1 ? args[commandIdx + 1] : undefined;
            const url = urlIdx !== -1 ? args[urlIdx + 1] : undefined;

            const argsIdx = args.indexOf("--args");
            const serverArgs = argsIdx !== -1 ? args.slice(argsIdx + 1).filter(a => !a.startsWith("--")) : undefined;
        ...
        default:
            ...
            console.log("  add <name> --command <cmd>    Add a stdio MCP server");
            console.log("  add <name> --url <url>        Add an HTTP MCP server");
            console.log("  remove <name>                 Remove an MCP server");
            console.log("  list                          List all MCP servers");
            console.log("\nOptions:");
            console.log("  --args <arg1> <arg2> ...      Arguments for stdio servers");
            console.log("  --env KEY=VALUE ...           Environment variables");
            console.log("\nExamples:");
            console.log("  nightcode mcp add filesystem --command npx --args -y @modelcontextprotocol/server-filesystem .");
            console.log("  nightcode mcp add remote --url http://localhost:3000/mcp");
            console.log("\n");
            break;
    }
...
function RouteErrorBoundary() {
    ...
    try {
        ...
        writeFileSync(join(dir, "error-boundary-crash.log"), error?.stack || error?.message || String(error));
    } catch { /* ignore */ }
    return (

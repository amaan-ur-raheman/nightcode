...
describe("Settings Module", () => {
    beforeEach(async () => {
        try { unlinkSync(TEST_SETTINGS_PATH); } catch {}
    });
    ...
    it("isConfirmationEnabled defaults to true", async () => {
        const { isConfirmationEnabled, saveSettings } = await import("../settings");
        saveSettings({});
        expect(isConfirmationEnabled()).toBe(true);
    });

    it("toggleConfirmations toggles the value", async () => {
        const { toggleConfirmations, isConfirmationEnabled, saveSettings } = await import("../settings");
        saveSettings({});
    ...
    it("isReasoningEnabled defaults to false", async () => {
        const { isReasoningEnabled, saveSettings } = await import("../settings");
        saveSettings({});
        expect(isReasoningEnabled()).toBe(false);
    });

    it("toggleReasoning toggles the value", async () => {
        const { toggleReasoning, isReasoningEnabled, saveSettings } = await import("../settings");
        saveSettings({});

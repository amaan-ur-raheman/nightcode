...
describe("BatchManager", () => {
    it("executes non-batchable tools immediately", async () => {
        ...
        const result = await batchManager.addRequest(
            "bash", { command: "echo hi" }, executor, "BUILD"
        );
    ...
    it("batches readFile requests", async () => {
        ...
        const promise1 = batchManager.addRequest(
            "readFile", { path: "a.ts" }, executor, "BUILD"
        );
        const promise2 = batchManager.addRequest(
            "readFile", { path: "b.ts" }, executor, "BUILD"
        );
        // Both should resolve
    ...
    it("notifies parallel results as each tool settles", async () => {
        ...
        const promise = batchManager.executeParallel(
            [
                { toolName: "bash", input: { command: "sleep 1" }, toolCallId: "slow-id", signal: slowController.signal },
                { toolName: "bash", input: { command: "echo fast" }, toolCallId: "fast-id", signal: fastController.signal },
            ],
            executor,
            "BUILD",
            undefined,
            undefined,
            (result) => settled.push(`${result.toolCallId}:${String(result.result)}`),
        );

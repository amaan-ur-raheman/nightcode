...
describe("SnapshotManager", () => {
    const SNAPSHOTS_FILE = join(homedir(), ".nightcode", "snapshots", "snapshots.json");

    beforeEach(async () => {
        try { unlinkSync(SNAPSHOTS_FILE); } catch {}
    });
    ...
    it("matches against stored snapshot", async () => {
        ...
        const result1 = await snapshotManager.match("test-snapshot", "hello world");
        expect(result1.match).toBe(true);

        // Second call with same value should match
        const result2 = await snapshotManager.match("test-snapshot", "hello world");
    ...
    it("detects mismatched snapshots", async () => {
        ...
        const result = await snapshotManager.match("test-snapshot", "changed value");

...
describe("MemoryManager with file locking", () => {
    ...
    it("successfully sets and deletes values while acquiring and releasing a file lock", async () => {
        ...
        expect(fsPromises.open).toHaveBeenCalledWith(expect.stringContaining("global.json.lock"), "wx");
        expect(mockHandle.close).toHaveBeenCalled();
        expect(fsPromises.writeFile).toHaveBeenCalled();
        expect(fsPromises.unlink).toHaveBeenCalledWith(expect.stringContaining("global.json.lock"));

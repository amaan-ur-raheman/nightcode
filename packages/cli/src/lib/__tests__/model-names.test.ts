...
describe('getModelName', () => {
    it('returns friendly name for known NIM models', () => {
        expect(getModelName('nvidia/nemotron-3-ultra-550b-a55b')).toBe(
            'Nemotron 3 Ultra 550B',
        );
        expect(getModelName('nvidia/deepseek-ai/deepseek-v4-pro')).toBe(
            'DeepSeek V4 Pro',
        );
        expect(getModelName('nvidia/qwen/qwen3.5-397b-a17b')).toBe(
            'Qwen 3.5 397B',
        );
    });

    it('returns friendly name for known Anthropic models', () => {
        expect(getModelName('claude-sonnet-4-20250514')).toBe(
            'Claude Sonnet 4',
        );
        expect(getModelName('claude-3-5-haiku-20241022')).toBe(
            'Claude 3.5 Haiku',
        );
    });
    ...
    it('returns formatted name for unknown models', () => {
        expect(getModelName('some-unknown/model-v1')).toBe('Model V1');
        expect(getModelName('custom-model')).toBe('Custom Model');
    });

    it('returns formatted name when it matches no known model', () => {

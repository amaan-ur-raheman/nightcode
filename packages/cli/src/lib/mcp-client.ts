...
async function connectServer(
    ...
    debug.log('mcp', `Connecting to server: ${name}`, {
    ...
    if (config.url) {
        ...
        if (!config.command) {
            throw new Error(
                'MCP client: missing command when url is not provided',
            );
        }
    ...
    await Promise.allSettled(
        servers.map(async ({ name, config }) => {
            if (!mcpScope.isServerActive(name)) {
                debug.log(
                    'mcp',
                    `Skipping server ${name} (not in session scope)`,
                );
            ...
            try {
                ...
                debug.log(
                    'mcp',
                    `Discovered ${result.tools.length} tools from ${name}`,
                );

...
export async function exportSession(sessionId: string): Promise<ExportedSession> {
    const res = await apiClient.export.session[":id"].$get({ param: { id: sessionId } });
...
export async function importSession(filePath: string): Promise<{ id: string; title: string }> {
    ...
    if ("sessions" in data) {
        ...
        for (const s of allSessions.sessions) {
            ...
            if (!res.ok) {
                const body = await res.json() as { error?: string };
                throw new Error(body.error ?? `Import failed for "${s.title}" (${res.status})`);
            }
...
export function exportToFile(filePath: string, data: ExportedSession | ExportedAllSessions): void {

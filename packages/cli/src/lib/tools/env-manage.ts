...
export async function envManageTool(input: unknown) {
    const { action, key, value, file } = toolInputSchemas.envManage.parse(input);
    ...
    switch (action) {
        case "read": {
            ...
            return { content, path: envFilePath, lines: content.split("\n").length };
        }
        ...
        case "list": {
            ...
            return {
                variables: vars.map((v) => ({ key: v.key, value: v.value, line: v.line })),
                path: envFilePath,
                count: vars.length,
            };
        }
        ...
        case "add": {
            ...
            if (value === undefined) return { error: "value is required for add action" };
            ...
            if (vars.some((v) => v.key === key)) {
                return { error: `Variable ${key} already exists. Use 'update' action instead.` };
            }

            const newLine = content.length > 0 && !content.endsWith("\n") ? "\n" : "";
            const updated = `${content}${newLine}${key}=${value}\n`;

            await undoManager.backup(resolved, "envManage", `Add ${key} to ${envFilePath}`);
            await writeFile(resolved, updated, "utf-8");

            return { success: true, action: "add", key, value, path: envFilePath };
        }
        ...
        case "update": {
            ...
            if (!found) {
                return { error: `Variable ${key} not found. Use 'add' action instead.` };
            }

            await undoManager.backup(resolved, "envManage", `Update ${key} in ${envFilePath}`);
            await writeFile(resolved, lines.join("\n"), "utf-8");

            return { success: true, action: "update", key, value, path: envFilePath };
        }
        ...
        case "delete": {
            ...
            await undoManager.backup(resolved, "envManage", `Delete ${key} from ${envFilePath}`);

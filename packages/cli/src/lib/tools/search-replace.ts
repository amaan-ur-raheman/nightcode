...
export async function searchReplaceTool(input: unknown) {
    ...
    const diffs: { path: string; diff: string; replacements: number; updated: string }[] = [];
    ...
    for (const file of allMatches) {
        ...
        diffs.push({ path: relPath, diff: diffOutput, replacements: count, updated });
    }
    ...
    const diffSummary = diffs.map((d) => `--- ${d.path} (${d.replacements} replacements)\n${d.diff}`).join("\n\n");

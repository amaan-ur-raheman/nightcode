...
export async function writeResult(graphId: string, taskId: string, result: string): Promise<void> {
    const filePath = join(WORKSPACE_ROOT, graphId, "results", `${taskId}.json`);
    await writeFile(filePath, JSON.stringify({ taskId, result, timestamp: Date.now() }), "utf-8");
}

export async function readResult(graphId: string, taskId: string): Promise<string | null> {
    try {
        const filePath = join(WORKSPACE_ROOT, graphId, "results", `${taskId}.json`);
...
export async function readAllResults(graphId: string): Promise<Record<string, string>> {
...
export async function shareContext(graphId: string, key: string, value: unknown): Promise<void> {
    const filePath = join(WORKSPACE_ROOT, graphId, "context", `${key}.json`);
    await writeFile(filePath, JSON.stringify(value, null, 2), "utf-8");
}

export async function readContext(graphId: string, key: string): Promise<unknown> {
    try {
        const filePath = join(WORKSPACE_ROOT, graphId, "context", `${key}.json`);
        const content = await readFile(filePath, "utf-8");
...
export async function setProjectInfo(graphId: string, info: string): Promise<void> {
    const filePath = join(WORKSPACE_ROOT, graphId, "context", "project-info.md");

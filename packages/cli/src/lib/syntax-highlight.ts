...
function detectLanguage(code: string, langHint?: string): string {
    ...
    if (
        trimmed.match(
            /(public|private|protected)\s+(static\s+)?(class|void|int)/,
        )
    )
        return 'java';
...
function tokenize(code: string, lang: string): Token[] {
    ...
    while (i < code.length) {
        ...
        if (commentStart) {
            if (commentStart[1] && code.startsWith(commentStart[1], i)) {
                const endIdx = code.indexOf(
                    commentStart[1] === '/*' ? '*/' : '-->',
                    i + commentStart[1].length,
                );
                const end =
                    endIdx !== -1
                        ? endIdx + (commentStart[1] === '/*' ? 2 : 3)
                        : code.length;
        ...
        if (
            ...
            let j = i;
            while (j < code.length && /[0-9._xXeEabcdefABCDEF]/.test(code[j]!))
                j++;

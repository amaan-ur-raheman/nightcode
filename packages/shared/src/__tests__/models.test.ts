...
describe("SUPPORTED_CHAT_MODELS", () => {
    ...
    it("has expected NVIDIA models", () => {
        const nvidiaModels = SUPPORTED_CHAT_MODELS.filter((m) => m.provider === "nvidia");
        expect(nvidiaModels.length).toBeGreaterThan(10);
        expect(nvidiaModels.some((m) => m.id.includes("deepseek-v4"))).toBe(true);
    });

    it("has expected Anthropic models", () => {
        const anthroModels = SUPPORTED_CHAT_MODELS.filter((m) => m.provider === "anthropic");
        expect(anthroModels.some((m) => m.id.includes("claude-sonnet-4"))).toBe(true);
        expect(anthroModels.some((m) => m.id.includes("claude-3-5-haiku"))).toBe(true);
    });

    it("has expected OpenAI models", () => {
        const openaiModels = SUPPORTED_CHAT_MODELS.filter((m) => m.provider === "openai");
    ...
    it("has expected Groq models", () => {
        const groqModels = SUPPORTED_CHAT_MODELS.filter((m) => m.provider === "groq");
...
describe("findSupportedChatModel", () => {
    ...
    it("finds NVIDIA models", () => {
        const model = findSupportedChatModel("deepseek-ai/deepseek-v4-flash");
        expect(model).toBeDefined();
